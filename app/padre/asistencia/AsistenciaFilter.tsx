...
export function AsistenciaFilter({ bimestres, selectedBimestreId, hijoId }: AsistenciaFilterProps) {
  const router = useRouter();
  const [val, setVal] = useState(selectedBimestreId || "");

  const handleQuery = () => {
    if (val) {
      router.push(`/padre/asistencia?hijoId=${hijoId}&bimestreId=${val}`);
    }
  };
  ...
  return (
    <FilterPanel>
      <div className="space-y-2">
        ...
        <Select id="bimestre-select" value={val} onChange={(e) => setVal(e.target.value)}>
